...
import { collection, getDocs, deleteDoc, doc, getDoc } from "https://www.gstatic.com/firebasejs/12.11.0/firebase-firestore.js";
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    // KIỂM TRA TRẠNG THÁI KHÓA TÀI KHOẢN TỪ FIREBASE
    if (currentUser) {
        try {
            const userDoc = await getDoc(doc(db, "users", String(currentUser.id)));
    ...
    // xử lý hiển thị thông tin tài khoản trên thanh điều hướng
    if (currentUser) {
        const name = getName();
    ...
    const getFilteredArticles = () => {
        ...
        list = list.filter(a => String(a.userId) !== "1" && String(a.userId) !== "2");
        ...
        if (isMyPostMode) {
            ...
            list = list.filter(a => {
                ...
                if (currentUser && String(a.userId) === String(currentUser.id)) return true;
                return false;
            });
        }
    ...
    const render = () => {
        ...
        postsGrid.innerHTML = listToRender.map(a => `
            <article class="post-card">
                ...
                <div class="post-meta">
                   <span>Date: ${a.createdAt ? new Date(a.createdAt).toISOString().split('T')[0] : ''}</span>
    ...
    // RENDER PHÂN TRANG (GIỮ NGUYÊN LOGIC CŨ)
